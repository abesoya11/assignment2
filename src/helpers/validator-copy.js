class validator {
  static validateUserDetails(userDetails, userData) {
    if (userDetails.hasOwnProperty("email") && userDetails.hasOwnProperty("password") && userDetails.hasOwnProperty("newsPreferences") && this.validateEmailId(userDetails) && this.validateValuesInUserDetails(userDetails) && this.validateNewsPreferences(userDetails["newsPreferences"]) && this.validateUniqueUserEmailId(userDetails, userData)) {
      return {
        status: true,
  ...
  static validateValuesInUserDetails(userDetails) {
    if (userDetails.email != "" && userDetails.password != "" && userDetails.newsPreferences != "") {
      return true;
    }
    return false;
  }
